...
export default function ExamPage() {
  ...
  useEffect(() => {
    (async () => {
      console.log("[ExamPage] loading examId:", id);

      // ── Try Firebase first ────────────────────────────────────────────────
      let firestoreExam: Exam | null = null;
      let firestoreQs: typeof MOCK_QUESTIONS = [];

      try {
        firestoreExam = await getExam(id);
        console.log("[ExamPage] exam:", firestoreExam?.title ?? "not found");
      } catch (err) {
        console.warn("[ExamPage] getExam failed:", err);
      }

      if (firestoreExam) {
        try {
          firestoreQs = await getQuestions(id);
          console.log("[ExamPage] questions count:", firestoreQs.length);
          console.log("[ExamPage] questions:", firestoreQs);
        } catch (err) {
          console.error("[ExamPage] getQuestions failed:", err);
        }
      }

      // ── Use Firebase data if exam + questions both found ──────────────────
      if (firestoreExam && firestoreQs.length > 0) {
        setExam(firestoreExam);
        setQuestions(firestoreQs);
        setAnswers(new Array(firestoreQs.length).fill(-1));
        setIsMock(false);
        console.log("[ExamPage] ✓ using Firebase data");
      } else if (firestoreExam && firestoreQs.length === 0) {
        // Exam exists but questions missing — show error, NOT mock data
        console.error("[ExamPage] exam found but 0 questions. Check Firestore subcollection.");
        setExam(firestoreExam);
        setQuestions([]);
        setAnswers([]);
        setIsMock(false);
      } else {
        // Exam not in Firebase at all → use mock data as demo
        console.log("[ExamPage] exam not in Firebase → using mock data");
  ...
  if (phase === "loading") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] gap-3">
        ...
        />
        <p className="text-[16px]" style={{ color: "#4A5568" }}>กำลังโหลดข้อสอบ...</p>
      </div>
    );
  }
  ...
  if (!exam) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] gap-4 px-5 text-center">
        <p className="text-[18px] font-semibold text-gray-700">ไม่พบชุดข้อสอบนี้</p>
        <Link href="/" className="btn-primary text-sm">← กลับหน้าหลัก</Link>
      </div>
    );
  }

  // Exam found but no questions (import error or permission issue)
  if (!isMock && questions.length === 0 && phase === "intro") {
    return (
      <div className="flex flex-col items-center justify-center min-h-[70vh] gap-4 px-5 text-center">
        <div className="text-4xl mb-2">⚠️</div>
        <p className="text-[18px] font-semibold text-gray-700">{exam.title}</p>
        <p className="text-[16px]" style={{ color: "#DC2626" }}>
          ไม่พบข้อสอบในชุดนี้ (0 ข้อ)
        </p>
        <p className="text-[14px]" style={{ color: "#4A5568" }}>
          กรุณาตรวจสอบ Firestore Console → exams/{id}/questions
        </p>
        <Link href="/exams" className="btn-primary text-sm">← กลับคลังข้อสอบ</Link>
      </div>
    );
  }
  ...
  if (phase === "intro") {
    return (
      <div className="min-h-screen pb-16" style={{ backgroundColor: "#A8D5BF" }}>
        <div className="max-w-lg mx-auto px-5 pt-8">

          {/* Back link */}
          <Link
            href="/exams"
            className="inline-flex items-center gap-1.5 text-[16px] mb-8 transition-colors"
            style={{ color: "#4A5568" }}
          >
          ...
          {/* Subject badge */}
          <span
            className="inline-block text-[17px] font-bold tracking-wide px-2.5 py-1 rounded-full mb-3"
          ...
          {exam.description && (
            <p className="text-[16px] leading-relaxed mb-7" style={{ color: "#4A5568" }}>
              {exam.description}
            </p>
          )}
            ...
            {[
              ...
              <div key={i} className="flex items-center gap-8">
                <div>
                  ...
                  <div className="text-[17px] mt-0.5" style={{ color: "#4A5568" }}>{s.unit}</div>
                </div>
                {i < arr.length - 1 && (
          ...
          {/* Name input (optional) */}
          <div className="mb-7">
            <label className="label">ชื่อผู้สอบ <span style={{ color: "#5A6478" }}>(ไม่บังคับ)</span></label>
            <input
              className="input"
          ...
          {/* CTA */}
          <button className="btn-primary w-full py-3.5 text-[18px]" onClick={startExam}>
            เริ่มทำข้อสอบ
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor"
          ...
          {isMock && (
            <p className="text-center mt-4 text-[17px]" style={{ color: "#5A6478" }}>
    ...
    return (
      <div className="min-h-screen pb-16" style={{ backgroundColor: "#A8D5BF" }}>
        <div className="max-w-lg mx-auto px-5 pt-8">
          ...
          >
            <p
              className="text-[16px] font-bold tracking-[0.14em] uppercase mb-4"
            ...
            <p className="text-[17px] font-semibold mb-5" style={{ color: g.accent }}>
              {pct}% · {g.label}
            </p>
            ...
            >
              <div>
                ...
                <div className="text-[17px]" style={{ color: "#4A5568" }}>ถูก</div>
              </div>
              <div>
                <div className="text-[1rem] font-bold text-red-500">{wrong}</div>
                <div className="text-[17px]" style={{ color: "#4A5568" }}>ผิด</div>
              </div>
              {skipped > 0 && (
                <div>
                  <div className="text-[1rem] font-bold" style={{ color: "#4A5568" }}>{skipped}</div>
                  <div className="text-[17px]" style={{ color: "#4A5568" }}>ข้าม</div>
                </div>
              )}
              <div>
                <div className="text-[1rem] font-bold text-gray-700">{formatTime(timeSpent)}</div>
                <div className="text-[17px]" style={{ color: "#4A5568" }}>เวลา</div>
              </div>
            </div>
          </div>
          ...
          {/* Review label */}
          <p className="text-[17px] font-bold tracking-[0.12em] uppercase mb-5" style={{ color: "#4A5568" }}>
            เฉลยและคำอธิบาย
          </p>
          ...
          <div className="space-y-4 pb-4">
            {questions.map((q, qi) => {
              ...
              const borderColor = isSkipped ? "#6B7280" : isCorrect ? "#0B6E65" : "#EF4444";
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-start gap-3 mb-3.5">
                    <div
                      className="w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0
                                 text-[17px] font-bold text-white mt-0.5"
                      style={{ backgroundColor: borderColor }}
                    >
                      {isSkipped ? "–" : isCorrect ? "✓" : "✗"}
                    </div>
                    <p className="text-[16px] font-semibold text-gray-900 leading-relaxed">
                      <span className="font-normal" style={{ color: "#4A5568" }}>ข้อ {qi + 1} · </span>
                      {q.text}
                    </p>
                  </div>
                  ...
                  <div className="space-y-1.5 ml-9">
                    {q.options.map((opt, oi) => {
                      ...
                      return (
                        <div
                          key={oi}
                          className="flex items-center gap-2.5 px-3 py-2 rounded-xl text-[18px]"
                          style={{
                        ...
                        >
                          <span
                            ...
                            style={{ color: isAnswer ? "#0B6E65" : isWrong ? "#DC2626" : "#5A6478" }}
                          >
                            {OPTS[oi]}.
                          </span>
                          <span className="flex-1">{opt}</span>
                          {isAnswer && (
                            <span className="text-[16px] font-bold flex-shrink-0" style={{ color: "#0B6E65" }}>
                              ✓ เฉลย
                            </span>
                          )}
                        </div>
                  ...
                  {q.explanation && (
                    <div
                      className="mt-3 ml-9 px-3 py-2.5 rounded-xl text-[18px] leading-relaxed"
    ...
    <div className="min-h-screen pb-28" style={{ backgroundColor: "#A8D5BF" }}>
      ...
      >
        ...
        <div className="max-w-lg mx-auto px-5 h-12 flex items-center justify-between">
          ...
          <div className="flex items-baseline gap-1">
            <span className="text-[18px] font-bold text-gray-900">{current + 1}</span>
            <span className="text-[18px]" style={{ color: "#4A5568" }}>/ {questions.length}</span>
          </div>

          {/* Title (truncated) */}
          <span className="text-[18px] truncate max-w-[120px]" style={{ color: "#4A5568" }}>
            {exam.title}
          </span>
          ...
          {exam.timeLimit > 0 ? (
            ...
            <span className="text-[18px]" style={{ color: "#4A5568" }}>
      ...
      <div className="max-w-lg mx-auto px-5 pt-6">
        ...
        <div className="flex items-center gap-2 mb-5">
          <span
            className="text-[17px] font-bold px-2.5 py-1 rounded-full"
            style={{ backgroundColor: "#EBF5F3", color: "#0B6E65" }}
          >
            {exam.subject}
          </span>
          <span className="text-[18px]" style={{ color: "#5A6478" }}>
            ข้อที่ {current + 1}
          </span>
        </div>

        {/* Question text */}
        <p className="text-[18px] font-semibold text-gray-900 leading-relaxed mb-7">
          {q.text}
        </p>
              ...
              >
                ...
                <div
                  className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0
                             text-[16px] font-bold transition-all duration-150"
                  style={{
                ...
                <span
                  className="text-[16px] leading-snug transition-colors duration-150"
                  style={{
